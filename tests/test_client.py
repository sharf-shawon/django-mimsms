...
def test_send_one_to_many_success(client, respx_mock):
    ...
    response = client.send_one_to_many(
        numbers=["8801700000000", "8801700000001"],
        message="Test bulk"
    )
    assert response.status_code == 200
...
def test_send_one_to_many_get_success(client, respx_mock):
    ...
    response = client.send_one_to_many_get(
        numbers=["8801700000000", "8801700000001"],
        message="Test bulk GET"
    )
    assert response.status_code == 200
...
def test_check_dlr_actual_call(client, respx_mock):
    respx_mock.get("https://api.mimsms.com/api/SmsSending/DlrCheck").mock(
        return_value=httpx.Response(200, json={
            "statusCode": 200,
            "status": "success",
            "responseResult": "Delivered"
        })
    )
    client.check_dlr(trxn_id="foo")
